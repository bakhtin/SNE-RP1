...
def download_from_vk(tree=False, *args, **kwargs):
	...
	if not tree:
		...
		fullpath = "/home/tree.bin"
	
	output = ''
	try:
		os.remove(CACHE_DIR + fullpath)
	except:
		pass
	for i in array:
		...
		for j in response:
			...
			f = open(CACHE_DIR + fullpath + "temp", 'ab')
			f.write(res)
			f.close()

	if tree == True:
		f = open(CACHE_DIR + fullpath + "temp", 'r')
		output = f.read()
		size = unpack(">i", output[:4])[0]
		f.close()
		#print size
		#f.write(output[4:4 + size])
		os.remove(CACHE_DIR + fullpath + "temp")
		return output[4:4 + size]

	f1 = open(CACHE_DIR + fullpath + "temp", 'r')
	f2 = open(CACHE_DIR + fullpath, 'wb')
	f2.write(f1.read(size))
	f1.close()
	f2.close()
	for i in filenames:
		os.remove(i)
